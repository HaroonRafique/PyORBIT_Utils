...
def plot_optics(sc, dd, filename):
	...
	for key, value in sorted(dd.iteritems()):
		...
		# ~ ay_ptc.append(ptc_optics[str(key)][3])		
				
		Dx_x.append(str(key))
		Dx.append(np.nanmean(dd[key]['D_x'][0][200:]))
		# ~ Dx_ptc.append(ptc_optics[str(key)][4])		
		
		Dy_x.append(str(key))
		Dy.append(np.nanmean(dd[key]['D_y'][0][200:]))
		# ~ Dy_ptc.append(ptc_optics[str(key)][5])		
...
dd = add_input_file(dd, './00_SEM_tomo/output/output.mat', '0%')
dd = add_input_file(dd, './00_SEM_tomo_10pc/output/output.mat', '10%')
dd = add_input_file(dd, './00_SEM_tomo_20pc/output/output.mat', '20%')
dd = add_input_file(dd, './00_SEM_tomo_30pc/output/output.mat', '30%')
main_label = 'SEM_Tomo_noSC'
sc = 'noSC'

# ~ dd = add_input_file(dd, './00_SEM_tomo_10pc_sc/output/output.mat', '10% SC')
# ~ dd = add_input_file(dd, './00_SEM_tomo_20pc_sc/output/output.mat', '20% SC')
# ~ dd = add_input_file(dd, './00_SEM_tomo_30pc_sc/output/output.mat', '30% SC')
# ~ main_label = 'SEM_Tomo'
# ~ sc = 'Slice-by-Slice'

# ~ dd = add_input_file(dd, './00_SEM_Initial/output/output.mat', 'Matched')
# ~ dd = add_input_file(dd, './00_SEM_Initial_Mismatch/output/output.mat', 'Matched 10%')
# ~ main_label = 'SEM_Matched_noSC'
# ~ sc = 'noSC'
...
plot_parameter(sc, dd, parameter = 'beta_x', filename = main_label, percentage = False, turns = turn_tot, legend_label = legend_label)#, ymin=13.5, ymax=17.5)
...
plot_parameter(sc, dd, parameter = 'D_x', filename = main_label, percentage = False, turns = turn_tot, legend_label = legend_label)#, ymin=1.6, ymax=3.1)
