...
for l in fin:  
    ...
    speedup.append(float(134280.)/float(l.split()[1]))
...
fig.tight_layout();
#~ plt.show();
plt.savefig('Runtimes_time_per_cpu_log.png', dpi = 800);
...
plt.title("Speedup factor per CPU (w.r.t. 1 CPU) for PyORBIT on HPC-Batch");
...
# ~ plt.title("Speedup factor per CPU (c.f. 1 CPU) for PyORBIT on HPC-Batch");
...
# ~ fig.tight_layout();
# ~ plt.savefig('Speedup_per_cpu_log.png', dpi = 800);
