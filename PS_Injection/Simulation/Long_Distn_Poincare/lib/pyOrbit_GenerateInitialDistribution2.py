...
class LongitudinalJohoDistributionSingleHarmonic():
	...
	def __init__(self, Parameters_dict, Joho_order):
		...
		self.sigma_dE = np.sqrt( ((self.beta**2 * self.energy * self.rf_voltage) / (self.slip_factor * self.harmonic_number  * pi)) * ( np.cos(self.sigma_dphi + self.phi_s) - np.cos(self.phi_s) + self.sigma_dphi * np.sin(self.phi_s) ) )
	...
	def is_inside_limiting_countour(self, phi, dE):
		...
		H = abs( ((self.harmonic_number * self.slip_factor * dE**2) / (2*self.beta**2 * self.energy))  +  (self.rf_voltage / (2*pi) ) * (np.cos(phi) - np.cos(self.phi_s) + (phi-self.phi_s)*np.sin(self.phi_s))) 
