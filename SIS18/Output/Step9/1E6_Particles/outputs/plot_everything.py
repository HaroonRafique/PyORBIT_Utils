...
def plot_parameter(parameter, filename, title=None, ylab=None, yunit='-', ymin=None, ymax=None, percentage = False, percentage_one = False):

	if percentage:
		print '\nPlotting ', parameter, ' percentage'
	elif percentage:
		print '\nPlotting ', parameter, ' factor'
	else:
		print '\nPlotting ', parameter
	...
	if percentage:
		...
		ax1.plot(particles['turn'][0], (particles[parameter][0]/particles[parameter][0][0]), label=parameter);
		ylabel = str(parameter + ' / ' + parameter + '_0 [-]')
		ax1.set_ylabel(ylabel);
		figname = filename + '_' + parameter + 'factor.png'
...
plot_parameter(parameter = 'epsn_x', filename ='PS_Injection', yunit = 'm rad', percentage_one = True)
