...
for i in xrange(min_particle,n_part):
    if horizontal:
        ...
        # ~ max_amps_x.append(max(np.array(df_data['x'].iloc[i::n_part])))                  # MAXIMUM AMPLITUDE
    else:
        z.append( const * np.array(df_data['y'].iloc[i:(i+1):n_part])[0]) 
        # ~ max_amps_x.append(max(np.array(df_data['x'].iloc[i::n_part])))                  # MAXIMUM AMPLITUDE


# ~ for i in xrange(min_particle,30):
    # ~ print np.array(df_data['x'].iloc[i:(i+1):n_part])[0]
    # ~ max_amps_x.append(const * np.array(df_data['x'].iloc[i:(i+1):n_part])[0])   # * SQRT(betx0 / gamx0)
    # ~ max_amps_x.append(max(np.array(df_data['x'].iloc[i::n_part])))                  # MAXIMUM AMPLITUDE

# ~ for i in xrange(34,n_part):
    # ~ print np.array(df_data['x'].iloc[i:(i+1):n_part])[0]
    # ~ max_amps_x.append(const * np.array(df_data['x'].iloc[i:(i+1):n_part])[0])   # * SQRT(betx0 / gamx0)
    # ~ max_amps_x.append(max(np.array(df_data['x'].iloc[i::n_part])))                  # MAXIMUM AMPLITUDE
...
if horizontal:
	...
	ax.imshow(img, zorder=0, origin='upper', aspect='auto', extent=[0.0, 5.0, 0.2325, 0.3375])
	...
	ax.imshow(img, zorder=0, origin='upper', aspect='auto', extent=[0.0, 5.0, 0.05, 0.20625])
