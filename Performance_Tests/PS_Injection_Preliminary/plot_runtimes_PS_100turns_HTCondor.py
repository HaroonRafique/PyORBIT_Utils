...
if(plot_runtimes):
    ...
    plt.title("Wall-clock runtimes for PyORBIT on HTCondor 1E2 turns 5E4 particles");
    ax1.plot(cores, sec_No_SC, 'b-', label='No Space Charge');
    ax1.plot(cores, sec_SC, 'r-', label='2.5D Space Charge');
...
if(plot_runtimes):
    ...
    ax1.plot(cores, sec_No_SC_turn, 'b-', label='No Space Charge');
    ax1.plot(cores, sec_SC_turn, 'r-', label='2.5D Space Charge');
...
if (plot_speedup_cf_1cpu):
    ...
    one_cpu = sec_No_SC_H[0]
    print '\none cpu = ', one_cpu
    speedup = []
    x=[]
    y=[]
    j=cpus_H[0]
    for i in sec_No_SC_H:
...
if (plot_speedup_cf_1thread):
    ...
    for i in new_sperturn:
        ...
        # ~ y2.append(j*40*20)              # linear scaling with cores
...
if (plot_runtimes_per_turn):
    ...
    #~ ax2 = ax1.twinx();
    ax2.plot(cores, sperturn, 'r-', label='Per CPU');
    ax2.set_xlabel("cores [-]", color='k');
