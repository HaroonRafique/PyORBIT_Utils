...
get_dpp = np.sqrt(bunchtwissanalysis.getCorrelation(5,5)) / (bunch.getSyncParticle().gamma()*bunch.mass()*bunch.getSyncParticle().beta()**2)
get_bunch_length = 4 * np.sqrt(bunchtwissanalysis.getCorrelation(4,4)) / (speed_of_light*bunch.getSyncParticle().beta())
get_eps_z = 1e9 * 4 * pi * bunchtwissanalysis.getEmittance(2) / (speed_of_light*bunch.getSyncParticle().beta())
