...
def generate_initial_distribution(parameters, Lattice,output_file = 'Input/ParticleDistribution.in', summary_file = 'Input/ParticleDistribution_summary.txt', outputFormat='Orbit'):
        ...
        print 'generate_initial_distribution: start'
	...
	closedOrbity = {'y0': parameters['y0'], 'yp0': parameters['yp0']} 
        ...
        print 'generate_initial_distribution: initialise arrays'
	...
	dE = np.zeros(parameters['n_macroparticles'])
        ...
        print 'generate_initial_distribution: build distns'
	...
	if orbit_mpi.MPI_Comm_rank(comm) == 0:
		with open(output_file,"w") as fid:
			csv_writer = csv.writer(fid, delimiter=' ')
                        ...
                        print 'generate_initial_distribution: Enter root process'
			...
			for i in range(parameters['n_macroparticles']):
                ...
                print 'generate_initial_distribution: particle loop done'               
                print 'generate_initial_distribution: orbit formatting done'
		...
		print '\nCreated particle distribution with ' + str(parameters['n_macroparticles']) + ' macroparticles into file: ', output_file
                print 'generate_initial_distribution: summary file done'

	# ~ orbit_mpi.MPI_Barrier(comm)
        print 'generate_initial_distribution: MPI barrier done'

	return output_file
        
def generate_initial_distribution_FMA(parameters, output_file = 'Input/ParticleDistribution.in', summary_file = 'Input/ParticleDistribution_summary.txt', outputFormat='Orbit', triangular_grid = False):
	parameters['alphax0'] = Lattice.alphax0
	parameters['betax0']  = Lattice.betax0
	parameters['alphay0'] = Lattice.alphay0
	parameters['betay0']  = Lattice.betay0
	...
	Jx_sqrt = np.linspace(np.sqrt(emittance_x/gamma_x)/n_macroparticles_sqrt/10, np.sqrt(emittance_x/gamma_x), n_macroparticles_sqrt)
	Jy_sqrt = np.linspace(np.sqrt(emittance_y/gamma_y), np.sqrt(emittance_y/gamma_y)/n_macroparticles_sqrt/10, n_macroparticles_sqrt)
	x, y = np.meshgrid(Jx_sqrt*parameters['TransverseCut'], Jy_sqrt*parameters['TransverseCut'])
...
def generate_initial_distribution_FMA_J(parameters, output_file = 'Input/ParticleDistribution.in', summary_file = 'Input/ParticleDistribution_summary.txt', outputFormat='Orbit', triangular_grid = True):
	parameters['alphax0'] = Lattice.alphax0
	parameters['betax0']  = Lattice.betax0
	parameters['alphay0'] = Lattice.alphay0
	parameters['betay0']  = Lattice.betay0
...
def generate_initial_distribution_y02(parameters, Lattice, output_file = 'Input/ParticleDistribution.in', summary_file = 'Input/ParticleDistribution_summary.txt', outputFormat='Orbit',orientation='x'):
