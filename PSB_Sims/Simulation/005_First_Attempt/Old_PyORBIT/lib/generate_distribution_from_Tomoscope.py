...
def generate_initial_distribution(parameters, Lattice=None, output_file='ParticleDistribution.in', outputFormat='pyOrbit',
	...
	#************************ PUT OFFSET IN z MANUALLY TO TEST RF PHASE************************
	#z = t_rand * speed_of_light * beta *1e-9 #convert from ns to m
	dphase = 1 # delta phase from pi (in rad) as from RF table
	z = t_rand * speed_of_light * beta *1e-9 + p['circumference'] * dphase / (2 * np.pi) #convert from ns to m
	print('z offset:', p['circumference'] * dphase / (2 * np.pi))
